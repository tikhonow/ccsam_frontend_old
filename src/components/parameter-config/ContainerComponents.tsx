...
import { Receiver, Room, Source, Surface } from "../../objects";
...
type PropertyRowInputElement = ({
                                  value,
                                  onChange,
                                  options
                                }: { value: any, onChange: any, options?: Option[] }) => JSX.Element;
...
export const createPropertyInput = <T extends Containers>(
  ...
  return (
    <PropertyRow>
      ...
      <div>{ensureArray(property).map((prop, index) => <Element uuid={uuid} property={prop}
                                                                key={`${uuid}-${prop}-${index}`} {...{ options }} />)}</div>
    </PropertyRow>
  );
};
