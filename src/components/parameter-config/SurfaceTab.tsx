...
const MaterialButton = ({ uuid }: { uuid: string }) => {
  ...
  return (
    <PropertyButton label="Material" buttonLabel={name} event="OPEN_MATERIAL_DRAWER" args={undefined}
...
const Scattering = ({ uuid }: { uuid: string }) => {
  ...
  return (
    <PropertyRowFolder label="Scattering" open={open} onOpenClose={toggle}>
      <PropertyNumberInput uuid={uuid} label="Scattering Coefficient"
                           tooltip="Sets this surface's scattering coefficient" property="scatteringCoefficient" />
    </PropertyRowFolder>
  );
};
