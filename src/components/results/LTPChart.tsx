...
const Chart = ({ uuid, width = 1000, height = 200, events = false }: LTPChartProps) => {
  const {
    info,
    data: _data,
    from
  } = useResult(state => pickProps(["info", "data", "from"], state.results[uuid] as Result<ResultKind.LevelTimeProgression>));
...
export const LTPChart = ({ uuid, width = 400, height = 300, events = false }: LTPChartProps) => {
  ...
  const {
    ...
    info,
    from
  } = useResult(state => pickProps(["name", "info", "from"], state.results[uuid] as Result<ResultKind.LevelTimeProgression>));
