...
export const SaveDialog = () => {
  const {
    ...
    saveDialogVisible,
    set
  } = useAppStore(store => pickProps(["projectName", "saveDialogVisible", "set"], store));
      ...
      <div className={Classes.DIALOG_FOOTER}>
        <div className={Classes.DIALOG_FOOTER_ACTIONS}>
          <Button onClick={() => set(store => {
            store.saveDialogVisible = false;
          })}>
            Cancel
          </Button>
          <AnchorButton intent={Intent.SUCCESS} onClick={() => emit("SAVE", () => set(store => {
            store.saveDialogVisible = false;
          }))}>
            Save
          </AnchorButton>
