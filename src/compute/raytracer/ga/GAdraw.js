...
GA.prototype.scaleX = function(x) {
  return (x) * this.scaler + this.center.x;
};
GA.prototype.scaleY = function(y) {
  return (y) * this.scaler + this.center.y;
};
GA.prototype.scaleXinv = function(x) {
  return (x - this.center.x) / this.scaler;
};
GA.prototype.scaleYinv = function(y) {
  return (y - this.center.y) / this.scaler;
};
...
GA.prototype.drawExtras = function() {
  ...
  } else if (this.ETC_canvas) {  // Energy-time curve
  ...
  for (i = 0; i < this.rayPaths.length; i++) {
    ...
    if (this.rayPaths[i].type == IMAGE_SOURCE_RAY) {
      if (this.beamVisualization == BEAM_BRANCH) {
        ...
        drawRay = true;
    } else if (this.rayVisualizationOrder > -1) {
