...
import { addMoment, Directions, history } from "./history";
...
import { useAppStore, useContainer, useMaterial, useResult, useSolver } from "./store";
...
messenger.addMessageHandler("GET_VERSION", (acc, ...args) => {
  return cram.solvers;
});
...
messenger.addMessageHandler("SHOULD_ADD_RAYTRACER", (acc, ...args) => {
  ...
  console.log(raytracer.uuid);
  cram.solvers.rayt = raytracer.uuid;
    ...
    if (allowed[fileType(file.name)]) {
      ...
      switch (fileType(file.name)) {
        ...
        case "obj": {
          ...
          messenger.postMessage("SHOULD_ADD_RAYTRACER");
